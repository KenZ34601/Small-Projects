 ...
 * Author:         Ken Zhou                                                   *
 * Date:           02/24/17                                                   *
 * Description:    Nim is two player game. In this case, that means you and   *
 * the computer.  When you start the game, a random number of piles           *
 * consisting of a random number of stones is put together. Both players take *
 * turns. Each turn a player must remove one or more stones from a pile. The  *
 * player that takes the last stone(s) from the only remaining pile wins.     *
 * Usage:          Load the script and call Nim.init()                        *
 *----------------------------------------------------------------------------*/

var Nim = (function() {
    var maxHeaps = 7,                               // Maximum number of heaps possible
        maxTokens = 7,                              // Maximum number of tokens possible per heap
        dx = 490/maxHeaps,                          // Column and row sizes in pixels
        dy = 490/maxTokens,                         // Used for calculating position of Token objects
    ...
    var startGame = function() {
        ...
        for (i=0; i<tokens.length; i++) {
            for (var j=0; j<tokens[i].length; j++) {
                tokens[i][j] = new Token(((75 + i * dx)), (600 - (dy + j * dy)), i, j);
